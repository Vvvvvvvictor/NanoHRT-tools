...
from ..helpers.utils import deltaR, polarP4, configLogger, deltaPhi

import logging
logger = logging.getLogger('zbb')
configLogger('zbb', loglevel=logging.INFO)
PI = 3.141592653589793

class ZbbSampleProducer(HeavyFlavBaseProducer):
    ...
    def __init__(self, **kwargs):
        ...
        super(ZbbSampleProducer, self).__init__(channel='zbb', **kwargs)

    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        super(ZbbSampleProducer, self).beginFile(inputFile, outputFile, inputTree, wrappedOutputTree)
    ...
    def analyze(self, event):
        ...
        selected_jets = sorted(event.fatjets, key=lambda j: j.pt, reverse=True)
        if len(selected_jets) < 2:
            return False
        if (selected_jets[0].pt < 450) or (selected_jets[1].pt < 200) or (deltaPhi(selected_jets[0], selected_jets[1]) < PI / 2):
            return False
        probe_jets = selected_jets[:2]
        ...
        if self.year in ["2016APV", "2016"]:
            self.out.fillBranch("passHTTrig", (event.HLT_AK8PFJet425_SoftDropMass30>0) & (event.HLT_AK8PFJet500>0))
        else:
            self.out.fillBranch("passHTTrig", (event.HLT_AK8PFJet425_SoftDropMass30>0) & (event.HLT_AK8PFJet500>0))
